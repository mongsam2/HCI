...
export default function Test() {
  ...
  const handleCell300PartialPrint = async (
    lineId: number,
    cellIndex: number,
    hexData: string
  ) => {
    devices.map(async (device) => {
      await dotpadsdk.current?.displayGraphicLineData(
        device,
        lineId,
        cellIndex,
        hexData
